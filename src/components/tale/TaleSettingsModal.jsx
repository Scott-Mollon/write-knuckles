...
const TaleSettingsModal = ({
  ...
  const [compileOptions, setCompileOptions] = useState(
    () => getTaleCompilePreferences(tale, taleId).options,
  )
  ...
  const handleCompileSubmit = async (e) => {
    ...
    const optionsError = validateCompileOptions(compileOptions)
    if (optionsError) {
    ...
    try {
      ...
      const updatedTale = await updateCompilePreferences.mutateAsync({
        options: compileOptions,
        pageLayout: compilePageLayout,
      })

      const savedPrefs = getTaleCompilePreferences(updatedTale, taleId)
      onCompilePreferencesSaved?.({
        options: savedPrefs.options,
        pageLayout: {
          ...savedPrefs.pageLayout,
          showPageGuides: viewerPrefs.showPageGuides,
        },
      })
      onClose()
    } catch (err) {
      setError(err.message || 'Failed to save compile settings.')
    }
  }
      ...
      >
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-cream/60">{modalDescription}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        {activeTab === TABS.tale && showTaleTab && (
          <form onSubmit={handleTaleSubmit} className="space-y-5">
        ...
        {activeTab === TABS.compile && (
          <form onSubmit={handleCompileSubmit} className="space-y-5">
            <CompileSettingsPanel
              tale={tale}
              options={compileOptions}
              pageLayout={normalizePageLayout(compilePageLayout)}
              onOptionsChange={setCompileOptions}
              onPageLayoutChange={setCompilePageLayout}
            />
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="submit"
                disabled={updateCompilePreferences.isPending}
                className="border-2 border-bronze-dark px-6 py-2 font-ui text-sm uppercase text-bronze hover:border-bronze disabled:opacity-50"
              >
                {updateCompilePreferences.isPending ? 'Saving…' : 'Save Compile Settings'}
              </button>
              <button
                type="button"
                onClick={onClose}
