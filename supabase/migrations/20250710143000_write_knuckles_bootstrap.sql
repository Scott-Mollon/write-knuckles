...
-- Includes migrations 001–016 (image storage, tale covers, reference images, account deletion).
...
create trigger research_items_updated_at before update on write.research_items
  for each row execute function write.set_updated_at();
drop trigger if exists feature_requests_updated_at on write.feature_requests;
create trigger feature_requests_updated_at before update on write.feature_requests
  for each row execute function write.set_updated_at();
...
drop policy if exists "Authenticated users read feature requests" on write.feature_requests;
create policy "Authenticated users read feature requests" on write.feature_requests
  for select using (auth.uid() is not null);

drop policy if exists "Authenticated users create feature requests" on write.feature_requests;
create policy "Authenticated users create feature requests" on write.feature_requests
  for insert with check (auth.uid() = created_by);

drop policy if exists "Magazine admins update feature requests" on write.feature_requests;
...
drop policy if exists "Magazine admins delete feature requests" on write.feature_requests;
create policy "Magazine admins delete feature requests" on write.feature_requests
  for delete using (
    exists (select 1 from public."Admins" a where a.admin_id = auth.uid())
  );

drop policy if exists "Authenticated users read feature request votes" on write.feature_request_votes;
create policy "Authenticated users read feature request votes" on write.feature_request_votes
  for select using (auth.uid() is not null);

drop policy if exists "Authenticated users vote on feature requests" on write.feature_request_votes;
create policy "Authenticated users vote on feature requests" on write.feature_request_votes
  for insert with check (auth.uid() = user_id);

drop policy if exists "Authenticated users remove own votes" on write.feature_request_votes;
